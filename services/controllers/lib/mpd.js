 ...
 */
var MPD = function(_port, _host, _password){
     ...
     * @param {String} event_name - what sort of event to listen for. must be one of the following:  'Error', 'Event', 'UnhandledEvent', 'AuthFailure', 'DatabaseChanging', 'DatabaseUpdated', 'DataLoaded', 'OutputChanged', 'StateChanged', 'QueueChanged', 'PlaylistsChanged', 'PlaylistChanged','Connect', 'Disconnect'
     * @param {errorEventHandler|disconnectEventHandler|connectEventHandler|playlistsChangedEventHandler|queueChangedEventHandler|outputChangedEventHandler|stateChangedEventHandler|dataLoadedEventHandler|databaseChangingEventHandler|databaseUpdatedEventHandler|unhandledEventHandler|eventHandler|errorEventHandler} handler - function called when the given event happens
     */
    self.on = on;
   ...
   var _private = {
    ...
    function init(){
        ...
        var wSocket = net.connect({ port: _port,  host: _host}, onConnect);
       
        wSocket.on('data', function(obj){ 
            _private.raw_buffer += obj.toString();
            _private.responceProcessor.apply(this,arguments);
            //console.log("||---- webSocket event DATA ---->>> "+obj.toString());
        });
        wSocket.on('error', function(obj){ 
            console.log("||---- webSocket event ERROR ---->>> "+obj.toString()); 
        });
 ...
 */
/**
 * event handler for 'DatabaseUpdated' events
 * event hander callback for when the music database has finished updating and changes have been made to DB
 * @event DatabaseUpdated
 * @type {Object}
 * @callback DatabaseUpdatedEventHandler
 * @param {Object} [responce_event] -
 * @param {MPD} client - the client that this event happened on
 */
/**
